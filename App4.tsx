...
const App4: FunctionComponent<Props> = (props) => {
  ...
  const { increment, reset, count, calibrated, setCalibrated, setData, data } = React.useContext(CounterContext);
  const [tfReady, setTfReady] = useState(false);
  const [model, setModel] = useState<posedetection.PoseDetector>();
  const [typeOfServeDetector, setTypeOfServeDetector] = useState<tf.LayersModel>();
  ...
  useEffect(() => {
    async function prepare() {
      ...
      const model_tos = await tf.loadLayersModel( bundleResourceIO(model_json, model_weight) );
  ...
  const getOutputTensorWidth = () => { return isPortrait() || IS_ANDROID ? OUTPUT_TENSOR_WIDTH : OUTPUT_TENSOR_HEIGHT; };

  const getOutputTensorHeight = () => { return isPortrait() || IS_ANDROID ? OUTPUT_TENSOR_HEIGHT : OUTPUT_TENSOR_WIDTH; };
  ...
  const renderPose = () => {
    if (poses != null && poses.length > 0) {
      const keypoints = poses[0].keypoints.filter((k) => (k.score ?? 0) > MIN_KEYPOINT_SCORE).map((k) => {
        // Flip horizontally on android or when using back camera on iOS.
        const flipX = IS_ANDROID || cameraType === Camera.Constants.Type.back;
        const x = flipX ? cameraLayoutWidth - k.x : k.x;
        const y = k.y;
        const cx = (x / cameraLayoutWidth) * (isPortrait() ? cameraLayoutWidth : cameraLayoutHeight);
        const cy = (y / cameraLayoutHeight) * (isPortrait() ? cameraLayoutHeight : cameraLayoutWidth);
        return ( 
          <Circle key={`skeletonkp_${k.name}`} cx={cx} cy={cy} r="4" strokeWidth="2" fill="#00AA00" stroke="white" />
        );
      });
  ...
  const find_angle = (a: any, b: any, c: any) => {
    let radians = Math.atan2(c[0].y - b[0].y, c[0].x - b[0].x) -  Math.atan2(a[0].y - b[0].y, a[0].x - b[0].x);
  ...
  const serveTypeDetectionthreshold = (poses: any) => {
    ...
    if (poses && poses.length > 0) {
      ...
      var leftShoulder = keypoints.filter(function (item: any) { return item.name === "left_shoulder"; });
      var rightShoulder = keypoints.filter(function (item: any) { return item.name === "right_shoulder"; });
      var leftElbow = keypoints.filter(function (item: any) { return item.name === "left_elbow"; });
      var rightElbow = keypoints.filter(function (item: any) { return item.name === "right_elbow"; });
      var rightHip = keypoints.filter(function (item: any) { return item.name === "right_hip"; });
      var leftHip = keypoints.filter(function (item: any) { return item.name === "left_hip"; });
      var leftKnee = keypoints.filter(function (item: any) { return item.name === "left_knee"; });
      var rightKnee = keypoints.filter(function (item: any) { return item.name === "right_knee"; });
  ...
  const handleCameraStream = async ( images: IterableIterator<tf.Tensor3D>, updatePreview: () => void, 
  ...
  return (
    <SafeAreaView style={styles_external.main_view}>
      <HeaderWithText  text = {title} hideProfileSection = {true} navigation={navigation} />
