...
const TermsConditionScreen: FunctionComponent<Props> = props => {
  const {navigation, route} = props;
  const [logOutCheck, setLogOutCheck] = useState(false);
  const [deleteAccountCheck, setDeleteAccountCheck] = useState(false);
  const {isTermCheck, isPrivacyCheck, setTermCheck} =
    React.useContext(SettingContext);

  const termCheckFunction = () => {
    setTermCheck();
    navigation.replace('PrivacyPolicyScreen');
  };

  return (
    <ScreenWrapperWithHeaderTermsPrivacyPolicy title="TERMS AND CONDITION">
